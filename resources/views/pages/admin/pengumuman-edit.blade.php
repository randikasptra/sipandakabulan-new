...
@section('title', 'Edit Pengumuman | SIPANDAKABULAN')
    ...
    <div class="mb-6">
        <nav aria-label="breadcrumb" class="mb-4">
            <ol class="flex items-center space-x-2 text-sm">
                ...
                <li class="flex items-center gap-2">
                    <i class="bi bi-chevron-right text-gray-400 text-xs"></i>
                    <span class="text-gray-600">Edit Pengumuman</span>
                </li>
            </ol>
        </nav>
        ...
        <h2 class="text-2xl lg:text-3xl font-bold text-gray-800 flex items-center gap-3">
            <div class="w-10 h-10 bg-gradient-to-r from-blue-900 to-blue-700 rounded-lg flex items-center justify-center">
                <i class="bi bi-pencil text-white text-lg"></i>
            </div>
            Edit Pengumuman
        </h2>
    </div>
        ...
        <form action="{{ route('admin.pengumuman.update', $pengumuman->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                ...
                <textarea name="isi" rows="8"
                    ...
                    placeholder="Tuliskan isi pengumuman..." required>{{ old('isi', $pengumuman->isi) }}</textarea>
                @error('isi')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-5">
                ...
                @if ($pengumuman->file)
                    <div class="mb-2 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-gray-700">File saat ini: {{ basename($pengumuman->file) }}</span>
                        </div>
                        <a href="{{ env('SUPABASE_URL') }}/storage/v1/object/public/{{ env('SUPABASE_STORAGE_BUCKET') }}/{{ $pengumuman->file }}"
                ...
                <p class="text-gray-500 text-xs mt-1">Upload file baru untuk mengganti file lama. Format: PDF, DOC, DOCX,
                    JPG, PNG</p>
                @error('file')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                        ...
                        @foreach ($desas as $desa)
                            <label
                                ...
                                <input type="checkbox" name="desa_ids[]" value="{{ $desa->id }}"
                                    ...
                                    {{ in_array($desa->id, old('desa_ids', $selected)) ? 'checked' : '' }}>
